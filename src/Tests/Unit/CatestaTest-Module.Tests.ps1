...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module CatestaTest.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain CatestaTest.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "CatestaTest.psm1"
        } #validPSM1
